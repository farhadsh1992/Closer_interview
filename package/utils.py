...
class Estimate_Deeptime(Callback):
    ...
    def on_epoch_begin(self, batch,epoch, logs={}):
        
        end_time = abs(self.start_time - pendulum.now())*1
        estimatTime = abs(abs(end_time*self.epoch/(batch+1))-end_time)
       
        
        memoryUse = psutil.virtual_memory().percent
        cpuUse = psutil.cpu_percent(interval=1, percpu=True)
        
        run = ("["+str(batch)+"/"+str(self.epoch)+"][Time: "+str(end_time)+
                  '][Remain :'+str(estimatTime)+"][Memory: "
                  +str(memoryUse)+"][CPU: "+str(cpuUse)+"]")
        sys.stdout.write('\r'+run)
    
class Estimate_fasterD(Callback):
